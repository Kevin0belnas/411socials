...
//   const handleSave = async () => {
//   try {
//     // Prepare the data with proper status handling
//     const formattedData = {
//       ...form,
//       date: formatDateForMySQL(form.date),
//       items: form.items.map(item => ({
//         ...item,
//         // Convert empty string status to null for backend consistency
//         status: item.status === "" ? null : item.status
//       }))
//     };
    
//     console.log("Data being sent to backend:", JSON.stringify(formattedData, null, 2));
...
//   // Status badge component - SIMPLIFIED and FIXED
...
//                                   <span className="truncate max-w-xs">{vendor.address}, {vendor.zipcode}</span>
//                                 </div>
//                               </div>
//                             </td>
//                             <td className="p-4">
...
//                                 className="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent"
//                                 name="status"
//                                 value={item.status || ''} // Ensure it's never undefined
//                                 onChange={(e) => handleItemChange(i, e)}
//                               >
...
export default function PurchaseOrder() {
  ...
  // Group orders by vendor when orders change and handle pagination
  useEffect(() => {
    if (orders.length > 0) {
      const vendorsMap = new Map();
      ...
      setTotalPages(Math.ceil(vendorsArray.length / itemsPerPage));
      
      // Expand all vendors by default for current page
      const currentVendors = getCurrentPageVendors(vendorsArray);
      setExpandedVendors(new Set(currentVendors.map(v => v.name)));
    } else {
      setVendors([]);
      setExpandedVendors(new Set());
      setTotalPages(1);
  ...
  const getCurrentPageVendors = (vendorsArray = vendors) => {
    const startIndex = (currentPage - 1) * itemsPerPage;
    const endIndex = startIndex + itemsPerPage;
    return vendorsArray.slice(startIndex, endIndex);
  };
  ...
  const goToPage = (page) => {
    if (page >= 1 && page <= totalPages) {
      setCurrentPage(page);
      // Reset expanded vendors for new page
      const currentVendors = getCurrentPageVendors();
      setExpandedVendors(new Set(currentVendors.map(v => v.name)));
    }
  };
    ...
    <>
      <div className="min-h-screen bg-gray-50 p-4">
        <div className="max-w-8xl mx-auto">
          ...
          {vendors.length > 0 && (
            <div className="bg-white rounded-xl shadow-sm p-4 mb-4 flex flex-col sm:flex-row justify-between items-center gap-4">
              <div className="flex items-center gap-4">
                <span className="text-sm text-gray-700">
                  Showing {startIndex + 1}-{endIndex} of {vendors.length} vendors
                </span>
                <select
                  value={itemsPerPage}
          ...
          {vendors.length > 0 && (
            <div className="bg-white rounded-xl shadow-sm p-4 mt-4 flex flex-col sm:flex-row justify-between items-center gap-4">
